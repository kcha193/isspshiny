...
datRaw <- read_spss("data/ISSP2003_National_Identity_II_data_set.sav")
...
fullNameForSelect <-
  fullNameForSelect[1:grep("How close do you feel to your ethnic group",
                           fullNameForSelect)]
...
dat <-
  datRaw %>% select(Q33, Q32) %>%   
  rename( gender = Q33, 
          age =  Q32) %>%   as.data.frame()
...
fullNameList$`2003` <- fullName
fullNameForSelectList$`2003` <- fullNameForSelect
datRawList$`2003` <- datRaw
datList$`2003` <- dat
