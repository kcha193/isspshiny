...
datRaw <- read_spss("data/ISSP2000_Environment_II_data_set.sav")
...
fullNameForSelect <-
  fullNameForSelect[1:grep("Taking everything into account, the world is getting better",
                           fullNameForSelect)]
...
dat <-
  datRaw %>% select( Q39, Q38) %>%   
  rename( gender = Q39, 
          age = Q38) %>%   as.data.frame()
...
fullNameList$`2000` <- fullName
fullNameForSelectList$`2000` <- fullNameForSelect
datRawList$`2000` <- datRaw
datList$`2000` <- dat
