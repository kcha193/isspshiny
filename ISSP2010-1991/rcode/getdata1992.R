...
datRaw <- read_spss("data/ISSP1992_Social_Inequality_II_data_set.sav")
...
fullNameForSelect <-
  fullNameForSelect[1:grep("DIAGRAM: CHAIRMAN OF A COMPANY",
                           fullNameForSelect)]
...
dat <-
  datRaw %>% select( Q31_1999, Q30_1999) %>%   
  rename( gender = Q31_1999, 
          age = Q30_1999) %>%   as.data.frame()
...
fullNameList$`1992` <- fullName
fullNameForSelectList$`1992` <- fullNameForSelect
datRawList$`1992` <- datRaw
datList$`1992` <- dat
