...
datRaw <- read_spss("data/ISSP1994_Family__Changing_Gender_Roles_II_data_set.sav")
...
fullNameForSelect <-
  fullNameForSelect[1:grep("Spouse work outside: kids left home",
                           fullNameForSelect)]
...
dat <-
  datRaw %>% select( Q29, Q28) %>%   
  rename( gender = Q29, 
          age = Q28) %>%   as.data.frame()
...
fullNameList$`1994` <- fullName
fullNameForSelectList$`1994` <- fullNameForSelect
datRawList$`1994` <- datRaw
datList$`1994` <- dat
