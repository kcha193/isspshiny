...
datRaw <- read_spss("data/ISSP2002_Family__Changing_Gender_Roles_III_data_set.sav")
...
fullNameForSelect <-
  fullNameForSelect[1:grep("Total number of people in household",
                           fullNameForSelect)]
...
dat <-
  datRaw %>% select(Q40, Q39) %>%   
  rename( gender = Q40, 
          age =  Q39) %>%   as.data.frame()
...
fullNameList$`2002` <- fullName
fullNameForSelectList$`2002` <- fullNameForSelect
datRawList$`2002` <- datRaw
datList$`2002` <- dat
